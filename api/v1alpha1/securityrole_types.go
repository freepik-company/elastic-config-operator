...
// SecurityRole manages OpenSearch Security Plugin roles via the _plugins/_security/api/roles API
type SecurityRoleSpec struct {
	...
	// ResourceSelector specifies the target OpenSearch cluster for security roles
	ResourceSelector ResourceSelector `json:"resourceSelector"`

	// Resources contains the security roles to apply, keyed by role name
	// Each key represents a role name, the value is the role definition
...
type SecurityRoleStatus struct {
	// Phase indicates the current phase of the SecurityRole.
	...
	// AppliedResources lists the names of the roles that were successfully applied to OpenSearch.
	// This is used to track which roles need to be deleted if they are removed from the spec.
	// +optional
	...
	// conditions represent the current state of the SecurityRole resource.
...
// +kubebuilder:printcolumn:name="Phase",type="string",JSONPath=".status.phase",description="Current phase of the SecurityRole"
...
// SecurityRole is the Schema for the securityroles API
...
type SecurityRole struct {
	...
	// spec defines the desired state of SecurityRole
	// +required
	Spec SecurityRoleSpec `json:"spec"`

	// status defines the observed state of SecurityRole
	// +optional
	Status SecurityRoleStatus `json:"status,omitzero"`
}
