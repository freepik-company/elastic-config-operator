...
// UpdateConditionSuccess updates the status of the NotificationChannel resource with a success condition
func (r *NotificationChannelReconciler) UpdateConditionSuccess(notificationChannel *v1alpha1.NotificationChannel) {
	...
	globals.UpdateCondition(&notificationChannel.Status.Conditions, condition)
}

// UpdateConditionKubernetesApiCallFailure updates the status of the NotificationChannel resource with a failure condition
func (r *NotificationChannelReconciler) UpdateConditionKubernetesApiCallFailure(notificationChannel *v1alpha1.NotificationChannel) {
	condition := globals.NewCondition(globals.ConditionTypeResourceSynced, metav1.ConditionTrue,
...
func (r *NotificationChannelReconciler) SetSyncing(ctx context.Context, resource *v1alpha1.NotificationChannel) {
...
func (r *NotificationChannelReconciler) SetReady(ctx context.Context, resource *v1alpha1.NotificationChannel, targetCluster string, appliedResources []string) error {
	now := metav1.Now()
	resource.Status.Phase = controller.PhaseReady
	resource.Status.Message = fmt.Sprintf("Successfully synced %d notification channels", len(appliedResources))
...
func (r *NotificationChannelReconciler) SetError(ctx context.Context, resource *v1alpha1.NotificationChannel, err error) {
