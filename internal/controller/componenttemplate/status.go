...
// UpdateConditionSuccess updates the status of the ComponentTemplate resource with a success condition
func (r *ComponentTemplateReconciler) UpdateConditionSuccess(componentTemplate *v1alpha1.ComponentTemplate) {
	...
	globals.UpdateCondition(&componentTemplate.Status.Conditions, condition)
}

// UpdateConditionKubernetesApiCallFailure updates the status of the ComponentTemplate resource with a failure condition
func (r *ComponentTemplateReconciler) UpdateConditionKubernetesApiCallFailure(componentTemplate *v1alpha1.ComponentTemplate) {
	condition := globals.NewCondition(globals.ConditionTypeResourceSynced, metav1.ConditionTrue,
...
func (r *ComponentTemplateReconciler) SetSyncing(ctx context.Context, resource *v1alpha1.ComponentTemplate) {
...
func (r *ComponentTemplateReconciler) SetReady(ctx context.Context, resource *v1alpha1.ComponentTemplate, targetCluster string, appliedResources []string) error {
	now := metav1.Now()
	resource.Status.Phase = controller.PhaseReady
	resource.Status.Message = fmt.Sprintf("Successfully synced %d component templates", len(appliedResources))
...
func (r *ComponentTemplateReconciler) SetError(ctx context.Context, resource *v1alpha1.ComponentTemplate, err error) {
